  ...
  <div class="warehouse-list">
    <div class="search-header">
      ...
      <div class="btn-group-top">
        <div class="icons" @click="dialogAdd()">
        ...
        <table class="table-content" border="0" cellspacing="0" cellpadding="0" v-else>
          ...
          <tbody v-if="!loading && tableList.length > 0">
            <tr v-for="item in tableList" :key="item.id">
              ...
              <td>{{ item.country }}</td>
              <td>
                <el-switch v-model="item.closed" @change="onChangeUsed($event,item)"></el-switch>
              </td>
              <td>
    ...
    <!-- 删除弹窗 -->
    <el-dialog title="Delete Warehouse" :visible.sync="dialogDeleteVisible" size="tiny">
...
export default {
  ...
  mounted() {
    ...
    this.headData = ["Warehouse Name", "Country", "Control", "Operation"];
  },
  methods: {
    // 返回路由
    goAfterIndex() {
    ...
    onEdit(event) {
      ...
      this.dialogTitle = "Update Warehouse";
    ...
    onChangeUsed(e, item) {
      this.loading = true;
      WarehouseListService.sSwitch(item.id)
        .then(res => {
    ...
    dialogAdd() {
      ...
      this.dialogTitle = "Add Warehouse";
      this.dialogWarehouseVisible = true;
    },
    // 弹窗确认
    dialogConfirm() {
      ...
      this.loading = true;
      WarehouseListService.addOneWarehouse(this.warehouse)
        .then(res => {
          if (res.code === -2) {
            throw res.message;
          }
    ...
    deleteInvitation() {
      const scope = this;
      scope.loading = true;
      WarehouseListService.deleteOneWarehouse(scope.currentID).then(
        res => {
          if (res.code === 201) {
            this.$message({
              message:
    ...
    getWarehouseList() {
      this.loading = true;
      WarehouseListService.listFilter(
        { warehouse_name: this.search_value },
        this.currentPage
      )
        .then(res => {
    ...
    // 弹窗必填
    required() {
      if (this.warehouse.warehouse_name === "") {
        this.$message({
