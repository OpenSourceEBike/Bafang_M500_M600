<template>
  <div class="order-schedule">
    <div class="header">
      ...
      <label class="title">Order Schedule</label>
    </div>
...
export default {
  ...
  created() {
    ...
    if (scope.orderId) {
      this.loading = true;
      OrderScheduleService.filterOrderSchedule(scope.orderId)
        .then(res => {
          if (res.data.code === -2) {
  ...
  methods: {
    formatSetpData(data) {
      ...
      scope.stepDatas = stepArr.map(res => {
        res.description =
          (res.description &&
            Core.Util.changeDateForm(res.description * 1000)) ||
          "";
    ...
    checkType(item) {
      ...
      switch (item.type) {
        // case "pi":
        // return scope.piType(item.status);
        case "delivery":
          return scope.deliveryType(item.status);
        case "clean":
          return scope.cleanType(item.status, item);
        case "shipment":
    ...
    cleanType(status, item) {
      switch (status) {
        case 0:
          item.description = "STATUS READY";
          return "wait";
        case 1:
          item.description = "EXPORT CLEAN";
          return "process";
        case 2:
          item.description = "IMPORT CLEAN";
          return "process";
        case 3:
          item.description = "FINISH CLEAN";
          return "success";
        default:
          return "wait";
      }
