<template>
  <div>
    <div class="header">
      ...
      <label class="title">{{ id ? 'Edit Stock Out' : 'Add Stock Out' }}</label>
        ...
        <div class="order-info">
          ...
          <div class="left">
            ...
            <div class="item">
              ...
              <el-select v-model="formData.purpose" class="input">
                <el-option
            ...
            <div class="item">
              ...
              <el-input v-model="formData.tracking_no" class="input" type="text"></el-input>
            </div>
            <div class="item">
              <label class="label">Customer Claims No.</label>
              <el-input v-model="formData.customer_claims_no" class="input" type="text"></el-input>
            </div>
            <div class="item">
              <label class="label">Repairing packages ticket No.</label>
              <el-input v-model="formData.repairing_packages_ticket_no" class="input" type="text"></el-input>
            </div>
          </div>
        </div>
    ...
    <el-dialog :title.sync="dialogTitle" :visible.sync="dialogAddVisible" class="dialog-region">
      ...
      <div class="item">
        <div class="item-box">
          ...
          <el-input v-model="materialData.qty" class="input" type="number" min="0"></el-input>
        </div>
      </div>
      <!-- <div class="item">
        <div class="item-box">
...
export default {
  ...
  created() {
    ...
    this.loading = true;
    StockOutHeaderService.findOne(this.id).then(res => {
    ...
    this.loading = true;
    StockOutService.filterAll(this.id).then(res => {
  ...
  methods: {
    ...
    selectCustomer(e) {
      const obj = this.customerList.find(item => item.id === e);
      if (obj) this.formData.customer_name = obj.name;
    },
    onSave() {
      const scope = this;
      ...
      this.$confirm("Cannot be modified after saving, continue?", "prompt", {
        ...
        .then(() => {
          if (this.headerRequired()) {
            ...
            console.log(
              JSON.stringify({
                nlOutStockHeader: this.formData,
                nlOutStocks: this.tableList
              })
            );

            StockOutService.saveHeader({
              nlOutStockHeader: this.formData,
              nlOutStocks: this.tableList
            })
              .then(res => {
                ...
                scope.$router.push({ name: "stockOut" });
              })
              .catch(err => {
                scope.$message({
                  message: err,
